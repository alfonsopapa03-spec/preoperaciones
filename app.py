...
# ==================== CREDENCIALES ====================
SUPABASE_DB_URL = "postgresql://postgres:Conejito200$@db.ogfenizdijcboekqhuhd.supabase.co:5432/postgres"

# ==================== CATÁLOGO DE MÁQUINAS ====================
# ⚠️ Cambia los nombres aquí según tus máquinas reales
MAQUINAS = [
    "Máquina 1",
    "Máquina 2",
    "Máquina 3",
...
# Sección 3 – ELEMENTOS DE PROTECCIÓN PERSONAL
ITEMS_EPP = [
...
<style>
    ...
    .item-label {
        font-size: 0.85rem; color: #333; padding: 0.3rem 0;
    }
    .badge-c {
...
class DB:
    ...
    def guardar_inspeccion(self, datos: dict, items: list) -> bool:
        """
        datos: dict con campos de inspecciones_preop
        items: lista de dicts {seccion, item_numero, descripcion, resultado}
        """
        try:
            c = self.conn()
            cur = c.cursor()
    ...
    def verificar_inspeccion_existente(self, fecha, maquina) -> bool:
        """Verifica si ya existe una inspección para esa máquina en esa fecha."""
        c = self.conn()
        try:
            cur = c.cursor()
            cur.execute(
...
def construir_items(prefix: str) -> list:
    """Recoge los valores del formulario y construye la lista de items."""
    items = []
    for i, desc in enumerate(ITEMS_ANTES_USO):
        key = f"{prefix}_au_{i}"
        resultado = st.session_state.get(key, "C")
        items.append({
            "seccion": "ANTES DE SU USO",
            "item_numero": i + 1,
            "descripcion": desc,
            "resultado": resultado
        })
    for i, desc in enumerate(ITEMS_EPP):
        ...
        resultado = st.session_state.get(key, "C")
        items.append({
            "seccion": "ELEMENTOS DE PROTECCIÓN PERSONAL",
            "item_numero": i + 1,
            "descripcion": desc,
            "resultado": resultado
        })
...
def render_items_seccion(seccion_label: str, items_lista: list, prefix: str, sufijo: str,
    ...
    """Renderiza los ítems de una sección con selectbox C/NC/N/A."""
...
def generar_excel(df_inspecciones: pd.DataFrame, db: "DB", titulo: str = "Inspecciones Preoperacionales") -> bytes:
    ...
    # Agregar conteos NC/C desde items si no están en el df
    for row_idx, (_, fila) in enumerate(df_inspecciones.iterrows(), start=3):
        insp_id = int(fila["id"])
        df_it   = db.obtener_items_inspeccion(insp_id)
        ...
        valores = {
            ...
            "maquina": fila.get("maquina", ""),
            "modelo": fila.get("modelo", ""),
            "marca": fila.get("marca", ""),
            "placa": fila.get("placa", ""),
            "trabajador": fila.get("trabajador", ""),
            "revisado_por": fila.get("revisado_por", ""),
    ...
    for _, insp in df_inspecciones.iterrows():
        ...
        for _, item in df_it.iterrows():
            ...
            vals = [
                insp_id,
                str(insp.get("fecha", "")),
                str(insp.get("maquina", "")),
                str(item.get("seccion", "")),
                int(item.get("item_numero", 0)),
    ...
    # Recopilar todos los items de las inspecciones del período
    todas_nc = []
    for _, insp in df_inspecciones.iterrows():
        df_it = db.obtener_items_inspeccion(int(insp["id"]))
        if not df_it.empty:
            for _, item in df_it.iterrows():
                todas_nc.append({
                    "seccion":    item.get("seccion", ""),
                    "descripcion":item.get("descripcion", ""),
                    "resultado":  item.get("resultado", "C"),
                })

    if todas_nc:
        df_nc_all = pd.DataFrame(todas_nc)
        total_insp = len(df_inspecciones)
        ranking_nc = df_nc_all.groupby(["seccion","descripcion"]).apply(
            lambda g: pd.Series({
                "num_nc": (g["resultado"] == "NC").sum(),
            })
...
def main():
    ...
    with tab1:
        ...
        with d1:
            fecha_insp = st.date_input("📅 Fecha", datetime.now(), key="n_fecha")
        with d2:
        ...
        # Alerta si ya existe inspección del día para esa máquina
        if db.verificar_inspeccion_existente(fecha_insp, maquina_sel):
            st.warning(f"⚠️ Ya existe una inspección registrada para **{maquina_sel}** el **{fecha_insp}**. Si continúas, se registrará una segunda.")
        ...
        st.markdown("<div class='seccion-titulo'>🦺 3A. ELEMENTOS DE PROTECCIÓN PERSONAL</div>",
                    unsafe_allow_html=True)
        render_items_seccion("EPP", ITEMS_EPP, "new", "epp")

        st.markdown("<div class='seccion-titulo'>⚡ 3B. SEGURIDAD ELÉCTRICA</div>",
                    unsafe_allow_html=True)
        render_items_seccion("SEGURIDAD ELÉCTRICA", ITEMS_ELECTRICA, "new", "elec")
        ...
        with c1:
            trabajador_inp    = st.text_input("👷 Trabajador",         placeholder="Nombre del operario",   key="n_trab")
        with c2:
            revisado_inp      = st.text_input("👤 Revisado por",       placeholder="Supervisor / Jefe",     key="n_rev")
        with c3:
            cliente_inp       = st.text_input("🏢 Cliente / Proyecto", placeholder="Nombre del proyecto",   key="n_cli")
        with c4:
            resp_mant_inp     = st.text_input("🔧 Resp. Mantenimiento",placeholder="Nombre del responsable",key="n_mant")

        e1, e2 = st.columns([1, 3])
        with e1:
            estado_inp = st.selectbox("🚦 Estado", ESTADOS_INSPECCION, key="n_estado")
        with e2:
            obs_inp    = st.text_area("💬 Comentarios / Observaciones",
        ...
        if st.button("💾 Guardar Inspección", type="primary", use_container_width=True, key="btn_guardar"):
            if not maquina_sel:
                ...
                datos = {
                    ...
                    "marca":  marca_inp,
                    "placa":  placa_inp,
                    "trabajador": trabajador_inp,
                    "revisado_por": revisado_inp,
                    "cliente_proyecto": cliente_inp,
                    "responsable_mantenimiento": resp_mant_inp,
    ...
    with tab2:
        ...
        with st.expander("🛠️ Filtros", expanded=True):
            ...
            with f1:
                fi = st.date_input("Desde", datetime.now() - timedelta(days=30), key="h_fi")
            with f2:
                ff = st.date_input("Hasta", datetime.now(), key="h_ff")
            with f3:
                maq_opts = ["Todas"] + MAQUINAS
                fm = st.selectbox("Máquina", maq_opts, key="h_fm")
            with f4:
                ftrab = st.text_input("Trabajador", key="h_trab")
            with f5:
                    ...
                    if not df_items_sel.empty:
                        ...
                        for sec in df_items_sel["seccion"].unique():
                            ...
                            for _, it in items_sec.iterrows():
                                badge = badge_resultado(it["resultado"])
                                st.markdown(f"&nbsp;&nbsp;&nbsp;{it['item_numero']}. {it['descripcion']} → **{badge}**")
    ...
    with tab3:
        ...
        try:
            ...
            with g4:
                ...
                df_nc_maq = df_s.groupby("maquina").agg(
                    prom_nc=("num_nc", "mean")
                ).reset_index().sort_values("prom_nc", ascending=True)
