...
    cols4_needed = ["conductor","placa","fecha","marca_temporal","_estado","*fallas_count","observaciones"]
...
    for ri, (*, row) in enumerate(df4.iterrows(), 3):
...
        Reporte y análisis de inspecciones pre-operacionales · Google Sheets
    
    """, unsafe_allow_html=True)
...
        crit_f = df["*estado"].str.contains("Críticas", na=False).sum() if "*estado" in df.columns else 0
...
                    data=excel_data,
                    file_name=f"{nombre_rep}*{datetime.now(BOGOTA_TZ).strftime('%Y%m%d*%H%M')}.xlsx",
...
                            men_c = g["*estado"].str.contains("Menores",na=False).sum() if "*estado" in g.columns else 0
                            crit_c = g["*estado"].str.contains("Críticas",na=False).sum() if "*estado" in g.columns else 0
...
                            data=excel_mes,
                            file_name=f"Inspecciones*{mes_sel_label.replace(' ','*')}*{datetime.now(BOGOTA_TZ).strftime('%Y%m%d*%H%M')}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        )
                else:
...
                st.markdown(f'{grupo}', unsafe_allow_html=True)
                cols_g = st.columns(3)
...
                        st.write("")
            st.markdown('📝 Observaciones y Hallazgos', unsafe_allow_html=True)
